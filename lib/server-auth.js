...
import { createSupabaseAdmin, createSupabaseServerClient } from './supabase';
...
export async function requireAgentRole() {
  ...
  // `db` is the service-key client for table access — `supabase` carries the
  // agent's session, so its writes run as the agent and RLS will refuse them.
  return { supabase, user, role: profile.role, db: createSupabaseAdmin() };
}
